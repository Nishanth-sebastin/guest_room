...
app.post("/guest/booked", (req, res) => {
  ...
  const status="Booked"
  BookingModel.findOneAndUpdate({guestname,ownername,roomid,"bookingdates.startdate":bookingdates.startdate,"bookingdates.enddate":bookingdates.enddate},{status,amount},{new:true}).then((result) => {
  res.json({message:result}) 
...
app.post("/owner/bookedroomdetails/", (req, res) => {
  ...
  BookingModel.find({ownername,status: { $in: ["Request Sent", "Booked"] } }).then((result)=>{
    console.log(result)
  })
});
