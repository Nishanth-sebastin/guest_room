...
//       {
//            RoomDetailsModel.find({roomid:result[i].roomid,ownername:result[i].ownername}).then((result2)=>{
//             res.json({result:result,result2})
//            })
//       }
//     }
...
app.post("/guest/get-booking-status", (req, res) => {
  ...
  BookingModel.find({ guestname }).then((result, err) => {
    if (result) {
      ...
      for (let i = 0; i < result.length; i++) {
        const promise = RoomDetailsModel.find({
        ...
        promise.then((result2) => {
          responseData.push({ result: result[i], result2 });
        });
      }
