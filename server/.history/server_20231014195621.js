...
app.post("/owner/login/", (req, res) => {
  ...
  OwnerRegModel.find({ email }).then((result, err) => {
    if (result[0].password == password) {
      res.json({ successMsg: "success", ownername: result[0].ownername });
    } else {
...
app.post("/owner/deleteroom/", (req, res) => {
  ...
  RoomDetailsModel.findOneAndDelete({ username,roomid }).then((result, err) => {
    res.json({ message: result });
  });
});
...
app.post("/guest/register/", (req, res) => {
  ...
  guestregmodel.create({ username, email, number, password }).then(
    (result, err) => {
      if (err) {
        console.log(err);
      } else {
...
app.post("/guest/request-booking", (req, res) => {
  // const { ownername, usernameguest,roomid,status,date } = req.body;

  // BookingModel.create({ ownername, guestname:usernameguest,roomid,status,date }).then((result, err) => {
  //   res.json({ message: result[0] });
  // });
});
