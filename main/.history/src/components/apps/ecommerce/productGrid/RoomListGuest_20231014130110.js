...
const RoomListGuest = ({ onClick }) => {
  ...
  const handleRoute=(username,roomid)=>{
    const usernameowner=localStorage.setItem('username',username)
        ...
        {loading ? (
          ...
          <>
            {rooms.map((room) => (
              ...
              >
                <BlankCard className="hoverCard">
                  <Typography component={Typography} onClick={handleRoute(room.username,room.roomid)} to={`/guest/${usernameguest}/room/detail/${room.username}}/${room.roomid}`}>
