      ...
      <Grid>
        {loading ? (
          <Grid container spacing={3}>
            ...
            <Slider slidesPerRow={3}  asNavFor={nav1} ref={(slider) => (slider2.current = slider)} {...settings}>
              {SliderData.map((step, index) => (
                <Box key={index} sx={{ p: 1, cursor: 'pointer', width: '100%' }}>
                  <img src={step.imgPath} alt={step.imgPath} width="100%"  height="100%" style={{ borderRadius: '5px' }} />
                </Box>
              ))}
            </Slider>
        )}
