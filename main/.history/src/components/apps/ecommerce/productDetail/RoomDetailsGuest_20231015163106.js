  ...
  useEffect(() => {
    axios.post('http://localhost:8080/owner/get-calender-reserved/', {
      ...
      if (response.data.message) {
        // Extract startdate and enddate from the API response and update reservedDates state
        const reservedDatesArray = response.data.message.map((booking) => {
          return {
            startdate: new Date(booking.bookingdates.startdate).toISOString().substr(0, 10),
            enddate: new Date(booking.bookingdates.enddate).toISOString().substr(0, 10),
          };
        });
