...
const AuthLoginGuest = ({ title, subtitle, subtext }) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      ...
      axios
        .post('http://localhost:8080/guest/login/', {
          email,
          password,
        })
        .then((response)=>{    
          ...
          if(response.data.successMsg==="success"){
            localStorage.setItem("usernameguest",response.data.username)
            const usernameguest=localStorage.getItem("usernameguest")
            navigate(`/guest/${usernameguest}/explore-rooms`)
          }
